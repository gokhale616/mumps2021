...
source("00/load_processed_data.R", chdir = TRUE)

# all these are where the plot data are synthesized
script_var <- c("data_cov_plot.R")
...
<<inc_data_line_plot, warning = FALSE, echo = FALSE, fig.cap = '\\capIncLine', out.width = outwidth[3]>>=

plot(incidence_line_plt)
...
plot(mumps_covariates_plt_data)
